...
public interface IListingRepository
{
    ...
    Task<Listing?> GetByIdIncludingDeletedAsync(Guid id, CancellationToken cancellationToken = default);
    ...
    Task<IEnumerable<Listing>> SearchAdminListingsAsync(
        string? keyword, ListingStatus? status, string? sortBy, int page, int pageSize, CancellationToken cancellationToken = default);
    Task<int> GetAdminTotalCountAsync(
        string? keyword, ListingStatus? status, CancellationToken cancellationToken = default);
